...
def display_nvd_data(cve_data):
    if cve_data and "vulnerabilities" in cve_data and len(cve_data["vulnerabilities"]) > 0:
        ...
        label_width = max(len("Description:"), len("Published:"), len("Base Score:"), len("Base Severity:"))
...
def display_epss_score(epss_score):
    if epss_score != "N/A":
        print(f"\nEPSS Score:    {float(epss_score) * 100:.2f}% Probability of exploitation in the wild (following publication).")
    else:
        print("\n[❌] No EPSS data found for this CVE ID.")
